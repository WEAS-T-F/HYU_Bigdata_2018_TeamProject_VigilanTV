...
def getPath():
    ...
    if request.method == 'POST':
        ...
        # 파일 stream 생성
        out = cv2.VideoWriter(capture_path_tmp + output_avi, fourcc, fps, (int(width), int(height)))
        # filename : 파일 이름
        # fourcc : 코덱
        # fps : 초당 프레임 수
        # width : 넓이
        ...
        while True:
            ...
            frame = cv2.putText(frame, timetext, (10, 100), cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, 2.5, (0, 0, 255), 4)
            ...
            if ret == True:
                ...
                for i in range(num):
                    ...
                    if IDcount[strID] == 180:
                        ...
                        #URL = "http://192.168.1.137:8081/vigilan"
                        #print("URL :", URL)
                        jsondata, data = make_json(capture_path, strID, saveTrackerCrop, image_start, image_detect, date_start,
...
def make_json(capture_path, car_id, car_capture_path, image_start, image_detect, date_start, time_start, date_detect,
              time_detect):
    carColor = []
    capture_path_tmp = "C:/Users/dlwlg/Desktop/capture_tmp/"
    ...
    if result is not None:
        ...
        #########################################################################################################
        # response
        data = {"capture_path": capture_path, "saveEvidenceCrop": image_detect,
                "saveTrackerCrop": car_capture_path, "final_plate": inputFileName, "final_save": outputFileName,
                "txt": resultOCR}

        real_data = {"date_start": date_start, "time_start": time_start, "date_detect": date_detect,
                     "time_detect": time_detect, "color": color, "plate_num": resultOCR, "image_start": image_start,
        ...
        path_data2 = json.dumps(data, ensure_ascii=False)
        print(path_data)
        #response = make_response(path_data)
        #response.headers['Content-Type'] = 'application/json;charset=utf-8'
        #print(str(response))
        print("path_data :", str(path_data))
        return path_data.encode('utf-8'), path_data2.encode('utf-8')
        ...
        # response
        data = {"capture_path": capture_path, "saveEvidenceCrop": image_start,
                "saveTrackerCrop": car_capture_path, "final_plate": inputFileName, "final_save": outputFileName,
                "txt": "X"}
